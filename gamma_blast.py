...
# --- API Credentials ---
API_KEY_DEFAULT = "n5r5fn7u84xxllmx"  # Default placeholder if env var not found
API_SECRET_DEFAULT = "jfm8fwtu1hiak1ubih9ykpz12h1lz7ev" # Default placeholder if env var not found
...
def get_nearest_weekly_expiry(instruments: list, underlying_prefix_str: str):
    ...
    logging.info(
        f"Searching for nearest weekly expiry for {underlying_prefix_str} among {len(instruments)} instruments.")
...
def get_relevant_option_details(instruments: list, atm_strike_val: float, expiry_dt: date,
    ...
    for i in range(-opt_count, opt_count + 1):
        ...
        if i == 0:
            ...
            key_suffix = f"itm{-i}"  # e.g., i=-1 -> itm1 (strike < ATM)
        else:
            key_suffix = f"otm{i}"  # e.g., i=1  -> otm1 (strike > ATM)
...
def fetch_historical_oi_data(kite_obj: KiteConnect, option_details_dict: dict,
    ...
    for option_key, details in option_details_dict.items():
        ...
        if not instrument_token:
            logging.warning(
                f"Missing instrument_token for {option_key} ({tradingsymbol}). Skipping historical data fetch.")
        ...
        try:
            ...
            logging.error(f"Error fetching historical OI for {tradingsymbol} (Token: {instrument_token}): {e}",
                          exc_info=True)
...
def calculate_oi_differences(raw_historical_data_store: dict, intervals_min: tuple):
    ...
    logging.debug(
        f"Calculating OI differences based on current time: {current_processing_time.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    ...
    for option_key, candles_list in raw_historical_data_store.items():
        ...
        for interval in intervals_min:
            ...
            if past_oi is not None:
                ...
                logging.debug(
                    f"Could not find past OI for {option_key} at {interval}m prior ({target_past_time.strftime('%H:%M:%S %Z')}). abs_oi_diff and pct_oi_change will be None.")
...
def generate_options_tables(oi_report: dict, contract_details: dict, current_atm_strike: float,
    ...
    if current_atm_strike is None:
        ...
        return Panel("[bold red]ATM Strike could not be determined. Tables cannot be generated.[/bold red]",
    ...
    for i in range(-num_strikes_each_side, num_strikes_each_side + 1):
        ...
        ce_strike_display = str(
            int(ce_contract.get('strike', strike_val)))  # Use actual strike from contract if available
    ...
    if (float(total_put_threshold_breached) / float(total_put_cells) > 0.5) or (
            float(total_call_threshold_breached) / float(total_call_cells) > 0.5):
        os.system('afplay /Users/amitheadspin/Downloads/alert.mp3')
...
def run_analysis_iteration(kite_conn: KiteConnect, nfo_instr: list, nearest_exp_date: date, symbol_prefix: str):
    ...
    try:
        ...
        if not current_atm_strike:  # Critical if ATM cannot be determined for this iteration
            ...
            return Panel(
                "[bold red]Error: Could not determine ATM strike. Check logs. Waiting for next refresh.[/bold red]",
                title="Update Error", border_style="red")
        ...
        if not nearest_exp_date:
            ...
            return Panel("[bold red]Error: Nearest expiry date not available. Critical error.[/bold red]",
                         title="Update Error", border_style="red")
        ...
        if not option_contract_details:
            ...
            return Panel(
                f"[bold yellow]Warning: Could not retrieve relevant option contracts for ATM {int(current_atm_strike)}. Waiting for next refresh.[/bold yellow]",
                title="Update Warning", border_style="yellow")
        ...
        return Panel(f"[bold red]An error occurred during data refresh: {e}. Check logs.[/bold red]",
                     title="Update Error", border_style="red")
...
def main():
    ...
    #     console.print(
    #         f"[yellow]Please set '{KITE_API_KEY_ENV_NAME}' and '{KITE_API_SECRET_ENV_NAME}' environment variables for live data.[/yellow]")
    ...
    try:
        ...
        console.print(
            f"[green]Successfully connected for user: {profile.get('user_id')} ({profile.get('user_name')})[/green]")
        ...
        if not nearest_expiry_date:
            console.print(
                f"[bold red]Could not determine nearest weekly expiry for {UNDERLYING_PREFIX}. Exiting.[/bold red]")
            logging.critical(f"Could not determine nearest weekly expiry for {UNDERLYING_PREFIX}.")
            sys.exit(1)  # Critical error
        console.print(
            f"Tracking options for expiry: [bold magenta]{nearest_expiry_date.strftime('%d-%b-%Y')}[/bold magenta]")

        console.print(
            f"Starting live updates. Refresh interval: {REFRESH_INTERVAL_SECONDS} seconds. Press Ctrl+C to exit.")
        console.print(
            f"Underlying: [bold cyan]{UNDERLYING_SYMBOL}[/bold cyan], Strike Difference: [bold cyan]{STRIKE_DIFFERENCE}[/bold cyan], Options Count per side: [bold cyan]{OPTIONS_COUNT}[/bold cyan]")
    ...
    # Handle specific Kite API exceptions during setup
        ...
        console.print(
            f"[bold red]Token Exception: {te}. This usually means an invalid or expired request_token or session issues. Please restart and re-login.[/bold red]")
        logging.critical(f"Token Exception: {te}", exc_info=True)
        sys.exit(1)
    except KiteConnect.exceptions.InputException as ie:
        console.print(
            f"[bold red]Input Error: {ie}. This could be due to an incorrect API Key/Secret or other parameters. Check configuration.[/bold red]")
